<!DOCTYPE html>
<html>
<head>
<style>
#customers {
  font-family: Arial, Helvetica, sans-serif;
  border-collapse: collapse;
  width: 100%;
}

#customers td, #customers th {
  border: 1px solid #ddd;
  padding: 8px;
}

#customers tr:nth-child(even){background-color: #f2f2f2;}

#customers tr:hover {background-color: #ddd;}

#customers th {
  padding-top: 12px;
  padding-bottom: 12px;
  text-align: left;
  background-color: #04AA6D;
  color: white;
}
</style>
</head>
<body>

<table id="customers">
  <tr>
  <td><h2>
  <?php $image_path = '/upload/easyschool.png'; ?>
  <img src="{{ public_path() . $image_path }}" width="100" height="50">

    </h2></td>
    <td>Employee Resigtration Details</td>
    <td>
    <p>Police Line, Pbana.</p>
    <p>Phone: 0731xxxxxx</p>
    <p>E-mail: support@ppschool.com</p>
    </td>
  
    
  </tr>
  

 
</table>


<table id="customers">
  <tr>
    <th width="10%">SL</th>
    <th width="35%">Employee DETAILS</th>
    <th width="55%">Employee DATA</th>
  </tr>

  <tr>
    <td>1</td>
    <td><b>Employee Image :</b></td>
    <td>
    <?php $image_path = '/upload/employee_images'; ?>
  <img src="{{ public_path() . $details->image }}" width="20" height="20">
        </td>
  </tr>

  <tr>
    <td>2</td>
    <td><b>Employee Name :</b></td>
    <td>{{$details->name }}</td>
  </tr> 

  <tr>
    <td>3</td>
    <td><b>Employee ID No :</b></td>
    <td>{{$details->id_no }}</td>
  </tr>

  <tr>
    <td>4</td>
    <td><b>Employee Father's Name   :</b></td>
    <td>{{$details->fname }}</td>
  </tr> 

  <tr>
    <td>5</td>
    <td><b>Employee Mother's Name :</b></td>
    <td>{{$details->mname }}</td>
  </tr>

  <tr>
    <td>6</td>
    <td><b>Employee Mobile Number :</b></td>
    <td>{{$details->mobile }}</td>
  </tr>

  <tr>
    <td>7</td>
    <td><b>Employee Address :</b></td>
    <td>{{$details->address }}</td>
  </tr>


  <tr>
    <td>8</td>
    <td><b>Employee Gender :</b></td>
    <td>{{$details->gender }}</td>
  </tr>


  <tr>
    <td>9</td>
    <td><b>Employee Religion :</b></td>
    <td>{{$details->religion }}</td>
  </tr>


  <tr>
    <td>10</td>
    <td><b>Employee Date Of Birth :</b></td>
    <td>{{ date('d-m-Y',strtotime($details->dob))  }}</td>
  </tr>

  <tr>
    <td>11</td>
    <td><b>Employee Joining Date :</b></td>
    <td>{{ date('d-m-Y',strtotime($details->join_date )) }}</td>
  </tr>


  <tr>
    <td>12</td>
    <td><b>Employee Designation :</b></td>
    <td>{{$details['designation']['title']  }}</td>
  </tr>


  <tr>
    <td>13</td>
    <td><b>Employee Salary :</b></td>
    <td>{{$details->selary  }}</td>
  </tr>
  

</table>
<br> <br>
    <i style="font-size: 10px; float: right;">Print Data : {{ date("d M Y") }}</i>

</body>
</html>